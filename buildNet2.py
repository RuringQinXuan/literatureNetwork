...
def Get_result(documents1,entities1):
	documents=str(documents1)
	entities=str(entities1)
	print(documents)
	print(entities)
	result_relation={}
	sentences_database={}
		...
		#to compare entity and sentence location in same pmid
			...
			while et < len(entityStops):
				...
				if entityStop < sentenceStop:
					et = et+1
					entityLoction=[entityStart-sentenceStart,entityStop-sentenceStart]
					...
					entityname=sentenceContent[entityLoction[0]:entityLoction[1]+1]
					if entityID not in dic.keys():
						dic[entityID]=[entityname]
					else:
						dic[entityID].append(entityname)
				else:
					if len(sentenceEntityList)>2:
						...
						for node in relations:
							if (node[0],node[1]) or (node[1],node[0]) not in result_relation.keys():
								result_relation[(node[0],node[1])]=[{"sentenceID":sentenceID,"entityLocation1":dic[node[0]],"entityLocation2":dic[node[1]],"sentence_content":sentenceContent}]
							else:
								result_relation[(node[0],node[1])].append({"sentenceID":sentenceID,"entityLocation1":dic[node[0]],"entityLocation2":dic[node[1]],"sentence_content":sentenceContent})
					dic={}                                        
					st=st+1
					sentenceEntityList=set()
