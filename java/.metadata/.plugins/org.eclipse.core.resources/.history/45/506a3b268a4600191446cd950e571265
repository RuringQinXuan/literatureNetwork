...
public class BulidNetworkTask extends AbstractTask implements ObservableTask{
	...
	public BulidNetworkTask(List<String> documents,List<String> entities){
	...
	public void run(TaskMonitor monitor) throws Exception {
		...
		String query_documents = String.join(",", documents);
		String query_entities = String.join(",", entities);
		...
		JSONObject result = HttpUtils.postJSON("localhosr:9000",
				args1);
